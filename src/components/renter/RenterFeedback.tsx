...
const AdminFeedback: React.FC<AdminFeedbackProps> = (props) => {
  ...
  const [firebaseConducteurId, setFirebaseConducteurId] = useState<string | null>(null);
  useEffect(() => {
    const auth = getAuth();
    const user = auth.currentUser;
    if (user) setFirebaseConducteurId(user.uid);
    else setFirebaseConducteurId(null);
  }, []);
  const conducteurId = props.conducteurId || location.state?.conducteur;
  const objetId = props.objetId || location.state?.objet;
  const conducteurToUse = firebaseConducteurId || conducteurId;
  console.log('[DEBUG] Props AdminFeedback:', { conducteurId, objetId, firebaseConducteurId, conducteurToUse });
  ...
  const fetchFeedbacks = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`https://qxmaqyf9jt.us-east-1.awsapprunner.com/feedbacks/byConducteur/${conducteurToUse}`);
  ...
  useEffect(() => {
    if (conducteurToUse) fetchFeedbacks();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        conducteur: conducteurToUse,
        texte,
        objet: selectedObjet || objet,
        document: document || null,
      };
      console.log('[DEBUG] Payload envoyé à l\'API:', payload);
      const res = await axios.post('https://qxmaqyf9jt.us-east-1.awsapprunner.com/feedbacks/addConducteur', payload);
      console.log('[DEBUG] Feedback envoyé avec succès:', res.data);

      // Création de la notification pour le propriétaire
      try {
        const notifAdmin = await axios.post('https://qxmaqyf9jt.us-east-1.awsapprunner.com/notifications/addNotification', {
          userId: 'admin',
          type: 'feedback',
          title: 'Nouveau feedback reçu',
          message: `Vous avez reçu un nouveau feedback de la part d'un locataire.`,
          link: `/admin/feedbacks/${res.data.feedbackId || ''}`,
          meta: {
            conducteur: conducteurToUse,
            objet: selectedObjet || objet,
            texte,
        ...
        console.log('[LOG] Notification admin créée:', notifAdmin.data);
      } catch (notifErr) {
        console.error('[Notification] Erreur lors de la création de la notification:', notifErr);
      }
      ...
      console.error('[DEBUG] Erreur lors de l\'envoi du feedback:', err);
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 flex flex-col h-[100vh]  bg-white rounded-lg shadow-lg border border-gray-200">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h2 className="text-2xl font-bold text-center flex-1 border-b pb-2">Feedbacks du Locataire</h2>
      </div>
      <div className='flex-1 overflow-y-auto flex flex-col gap-4  justify-between'>
          {selectedObjet && (
          ...
          {/* Affichage des feedbacks façon "bulle" type WhatsApp/Messenger */}
          <div className="flex-1 overflow-y-auto px-2 py-4 bg-gray-50 rounded mb-2 border">
            {loading ? (
              ...
              <div className="flex flex-col gap-2 px-1" style={{ maxHeight: '60vh' }}>
                {feedbacks
                  .filter(fb => !selectedObjet || fb.objet === selectedObjet)
                  .sort((a, b) => new Date(a.creatAt).getTime() - new Date(b.creatAt).getTime())
                  .map((fb) => {
                    // Correction : le message courant est celui du conducteur connecté
                    const isMine = fb.conducteur === conducteurToUse;
                    return (
                      <div
                        key={fb.feedbackId}
                      ...
                      >
                        <div
                          className={`rounded-2xl px-4 py-2 shadow border max-w-[75%] whitespace-pre-line break-words ${
                            isMine
                              ? 'bg-[#3EFEFE] text-black border-[#3EFEFE] rounded-br-none'
                              : 'bg-white border-gray-200 text-left rounded-bl-none'
                          }`}
                        >
                          <div className="text-xs text-gray-500 mb-1 flex items-center gap-2">
                            {fb.objet && !selectedObjet && (
                              <span className="font-semibold text-green-700">{fb.objet}</span>
                            )}
                            <span>{new Date(fb.creatAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</span>
                          </div>
                          <div className="mb-1">{fb.texte}</div>
                          {fb.document && (
                            <a href={fb.document} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline text-xs">Voir document</a>
                          )}
                        </div>
                      </div>
                    );
