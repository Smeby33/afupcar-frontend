...
const TextoRenter: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const location = useLocation();
  const navigate = useNavigate();
  const owner: any | undefined = location.state?.owner;
  const auth = getAuth();
  ...
  const [renterPhoto, setRenterPhoto] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchCommentaires = async () => {
      ...
      try {
        ...
        console.log('[DEBUG] Messages renvoyés:', res.data); // ou messages, ou rows selon ton code
      } catch (e: any) {
  ...
  useEffect(() => {
    if (currentUser?.uid) {
      axios.get(`https://qxmaqyf9jt.us-east-1.awsapprunner.com/renters/getRenter/${currentUser.uid}`)
        .then(res => {
          console.log('[DEBUG] Données reçues pour le locataire:', res.data);
          setRenterPhoto(res.data.photo || null);
        })
        .catch(() => setRenterPhoto(null));
    }
  }, [currentUser]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!id || !currentUser || !owner || !owner.id || (!newMessage.trim() && !file)) {
      setError("Impossible d'envoyer le message : informations propriétaire manquantes.");
      return;
    }
    console.log('[DEBUG] Envoi message avec owner.id =', owner.id, 'owner =', owner,'[DEBUG] Envoi message avec auteur-inter =', currentUser.uid);
    setSending(true);
    setUploading(true);
    let documentUrl = '';
    try {
      ...
      await axios.post('https://qxmaqyf9jt.us-east-1.awsapprunner.com/commentaires/addCommentaireInter', {
        id_conversation: id,
        auteur: currentUser.uid, // le locataire est l'auteur
        'auteur-inter': owner.id, // le propriétaire est l'interlocuteur
        message: newMessage,
        document: documentUrl,
      });
      console.log('[DEBUG] Message envoyé avec succès');
      ...
      try {
        const notifProprio = await axios.post('https://qxmaqyf9jt.us-east-1.awsapprunner.com/notifications/addNotification', {
          ...
          message: `Vous avez reçu un nouveau message de la part d'un locataire.`,
          link: `/owner/conversations/${id}`,
          meta: {
            id_conversation: id,
        ...
        console.log('[LOG] Notification propriétaire créée:', notifProprio.data);
      } catch (notifErr) {
    ...
    <div className="max-w-2xl mx-auto p-0 flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="flex items-center gap-3 bg-white border-b px-4 py-3 sticky top-0 z-10 shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-lg truncate">{owner?.companyname || owner?.fullname || owner?.nom || 'Propriétaire'}</div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4 flex flex-col-reverse" style={{ minHeight: 0, maxHeight: 'calc(100vh - 120px)' }}>
        {loading ? (
          ...
          <ul className="space-y-3">
            {commentaires.map((c) => {
              // Un message est "à moi" si c['auteur'] === currentUser?.uid
              // Un message est du propriétaire si c['auteur'] === owner?.id
              const isMe = c['auteur'] === currentUser?.uid;
              const isOwner = c['auteur'] === owner?.id;
              ...
              return (
                <li key={c.id_commentaire} className={`flex ${isMe ? 'justify-end' : 'justify-start'} w-full`} style={{ maxWidth: '100%' }}>
                  {/* Affichage avatar propriétaire seulement si c'est un message du propriétaire */}
                  {isOwner && !isMe && (
                    <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden mr-2 mt-auto">
                      {owner?.picture ? (
                        <img src={owner.picture} alt={owner.companyname || owner.fullname || owner.nom || 'Propriétaire'} className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-gray-400 text-base font-bold">{(owner?.companyname?.[0] || owner?.fullname?.[0] || '?').toUpperCase()}</span>
                      )}
                    </div>
                  )}
                  <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} w-auto`} style={{ maxWidth: '80%' }}>
                    <div className={`rounded-2xl px-4 py-2 shadow text-sm break-words ${isMe ? 'bg-blue-500 text-white' : 'bg-white text-gray-900 border'} relative max-w-xs md:max-w-md`} style={{ borderTopLeftRadius: isMe ? 16 : 4, borderTopRightRadius: isMe ? 4 : 16, minWidth: 60 }}>
                      {/* Affiche toujours le message texte */}
                      {c.message && (
                  ...
                  {/* Avatar du locataire pour ses propres messages */}
                  {isMe && (
                    <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden ml-2 mt-auto">
                      {renterPhoto ? (
                        <img src={renterPhoto} alt="Moi" className="w-full h-full object-cover" />
                      ) : (
